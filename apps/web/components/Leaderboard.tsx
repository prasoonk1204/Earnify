...
function getPodiumRowStyle(rank: number) {
  if (rank === 1) {
    return {
      className:
        "border-l-2 border-l-[#F59E0B] bg-[#16120a]/80 border-y border-y-[var(--color-border)]/30 border-r border-r-[var(--color-border)]/30",
    };
  }
  if (rank === 2) {
    return {
      className:
        "border-l-2 border-l-zinc-400 bg-zinc-900/65 border-y border-y-[var(--color-border)]/30 border-r border-r-[var(--color-border)]/30",
    };
  }
  if (rank === 3) {
    return {
      className:
        "border-l-2 border-l-zinc-500 bg-zinc-900/65 border-y border-y-[var(--color-border)]/30 border-r border-r-[var(--color-border)]/30",
    };
  }
  return {
...
export function Leaderboard({
  ...
  const [transientMovementByUserId, setTransientMovementByUserId] = useState<
  ...
  const fetchLeaderboard = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${apiBase}/api/campaigns/${campaignId}/leaderboard`,
        {
          credentials: "include",
        },
      );
  ...
  useEffect(() => {
    ...
    socket.on(
      ...
      (
        payload:
          | { campaignId?: string; leaderboard?: LeaderboardEntry[] }
          | undefined,
      ) => {
        if (
          !payload?.campaignId ||
        ...
        const previousRanks = new Map(
          entriesRef.current.map((entry) => [entry.userId, entry.rank]),
        );

        const movementByUserId: Record<string, RankMovement> = {};
        for (const entry of payload.leaderboard) {
          movementByUserId[entry.userId] = getMovementFromRanks(
            previousRanks.get(entry.userId),
            entry.rank,
          );
        }
      ...
      <ul className="space-y-3">
        {Array.from({ length: 5 }).map((_, index) => (
          <li
            ...
            className="rounded-sm border border-[var(--color-border)]/30 bg-[#0D0F14]/50 p-2.5 sm:p-3"
          >
            ...
            >
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <img
                      src={
                        entry.userAvatar ??
                        "https://placehold.co/64x64/e2e8f0/334155?text=U"
                      }
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
                    <div className="border border-[var(--color-border)]/60 bg-black/35 px-2.5 py-2">
                      ...
                      <p className="mt-0.5 font-semibold text-zinc-100">
                        {formatMetric(entry.xStats?.views ?? 0)}
                      </p>
                    </div>
                    <div className="border border-[var(--color-border)]/60 bg-black/35 px-2.5 py-2">
                      <p className="text-[10px] uppercase tracking-[0.08em] text-zinc-500">
                      ...
                      <p className="mt-0.5 font-semibold text-zinc-100">
                        {formatMetric(entry.xStats?.likes ?? 0)}
                      </p>
                    </div>
                    <div className="border border-[var(--color-border)]/60 bg-black/35 px-2.5 py-2">
                      <p className="text-[10px] uppercase tracking-[0.08em] text-zinc-500">
                      ...
                      <p className="mt-0.5 font-semibold text-zinc-100">
                        {formatMetric(entry.xStats?.replies ?? 0)}
                      </p>
                    </div>
                    <div className="border border-[var(--color-border)]/60 bg-black/35 px-2.5 py-2">
                      <p className="text-[10px] uppercase tracking-[0.08em] text-zinc-500">
                      ...
                      <p className="mt-0.5 font-semibold text-zinc-100">
                        {formatMetric(entry.xStats?.reposts ?? 0)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full border border-[var(--color-border)]/70 bg-black/45 px-4 py-3 text-left lg:w-[210px] lg:text-right">
                  <p className="text-[10px] uppercase tracking-[0.09em] text-zinc-500">
                    Score
                  </p>
