...
function FounderDashboardPage() {
  ...
  const stats = useMemo(() => {
    const totalBudget = campaigns.reduce(
      (sum, campaign) => sum + campaign.totalBudget,
      0,
    );
    ...
    const endedCount = campaigns.filter(
      (campaign) => classifyCampaignSegment(campaign) === "ended",
    ).length;
    const totalParticipants = campaigns.reduce(
      (sum, campaign) => sum + campaign.postCount,
      0,
    );
      ...
      <section className="mx-auto w-full max-w-7xl space-y-7 px-4 py-8 sm:px-6 lg:px-8">
        <header className="surface-card rounded-sm p-6 sm:p-8">
          ...
          <p className="mt-3 max-w-3xl text-sm text-zinc-400">
            Monitor campaign health, separate active and ended initiatives, and
            keep campaign operations clean.
          </p>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <article className="surface-card rounded-sm p-5">
            ...
            <p className="mt-2 text-2xl font-semibold text-zinc-100">
              {stats.totalBudget.toFixed(2)} XLM
            </p>
          ...
          {loadingCampaigns ? (
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              {Array.from({ length: 4 }).map((_, index) => (
                <div
                  key={`founder-campaign-skeleton-${index}`}
