...
const sectionTitleClass =
  "mt-2 text-2xl font-semibold tracking-tight text-zinc-100 sm:text-3xl";
...
function classifyCampaigns(campaigns: LandingCampaign[]) {
  ...
  const live = campaigns.filter(
    (campaign) => campaign.status === "ACTIVE" && !isEndedByDate(campaign),
  );
  ...
  const ended = campaigns.filter(
    (campaign) =>
      ...
      isEndedByDate(campaign),
  );

  const upcoming = campaigns.filter(
    (campaign) =>
      !live.some((entry) => entry.id === campaign.id) &&
      !ended.some((entry) => entry.id === campaign.id),
  );
...
const productTabs: Array<{
...
}> = [
  {
    ...
    label: "For Founders",
    title:
      "Launch campaigns with controlled budgets and transparent distribution.",
    description:
...
export default function HomePage() {
  ...
  const [activeProductTab, setActiveProductTab] =
    useState<ProductTabId>("founders");
  const [activeCampaignTab, setActiveCampaignTab] =
    useState<CampaignTabId>("live");
        ...
        <div className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
          <article className={`${sectionCardClass} p-7 sm:p-10`}>
            ...
            <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-400">
              Earnify combines campaign orchestration, creator verification,
              leaderboard scoring, and on-chain settlement in one modern
              workflow.
            </p>
            ...
            <div className="mt-5 space-y-5">
              <div>
                ...
                <p className="mt-1 text-3xl font-semibold tracking-tight text-zinc-100">
                  {segmented.live.length}
                </p>
              </div>
              <div>
                <p className={overlineClass}>Active Budget</p>
                <p className="mt-1 text-3xl font-semibold tracking-tight text-[var(--color-primary)]">
                  {liveBudget.toFixed(0)} XLM
                </p>
              </div>
              <div>
      ...
      <section className={`${shellClass} space-y-12`}>
        <section className={sectionCardClass}>
          ...
          <h2 className={sectionTitleClass}>
            Everything from campaign launch to creator payout.
          </h2>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            ...
            <div className="space-y-2.5">
              {selectedProduct.bullets.map((item) => (
                <div
                  key={item}
                  className="border border-zinc-800 bg-black/45 px-4 py-3 text-sm leading-6 text-zinc-300"
                >
                  {item}
                </div>
              ))}
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          <article className={`overflow-hidden rounded-sm ${surfaceClass}`}>
            ...
            <div className="p-6 sm:p-8">
              ...
              <p className={sectionBodyClass}>
                Earnify secures campaign operations with wallet-authenticated
                flows, contract-bound settlement, and explicit status
                transitions.
              </p>
            </div>
          </article>
          ...
          <article className={`overflow-hidden rounded-sm ${surfaceClass}`}>
            ...
            <div className="p-6 sm:p-8">
              ...
              <p className={sectionBodyClass}>
                Run concurrent campaigns, sync leaderboard updates, and settle
                creator pools with dependable state handling.
              </p>
            </div>
          </article>
        </section>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-zinc-500">
                Explore campaign inventory by status with real backend data.
              </p>
            </div>
              ...
              {(["live", "upcoming", "ended"] as CampaignTabId[]).map((tab) => (
                ...
                >
                  ...
                  {tab === "live"
                    ? segmented.live.length
                    : tab === "upcoming"
                      ? segmented.upcoming.length
                      : segmented.ended.length}
                  )
                </button>
              ))}
            </div>
        ...
        <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <article className={sectionCardClass}>
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-400">
              Live distribution of campaign platform requirements.
            </p>
            <div className="mt-5 space-y-2.5">
              {platformSpread.length === 0 ? (
                ...
                platformSpread.map(([platform, count]) => (
                  <div
                    key={platform}
                    className="flex items-center justify-between border border-zinc-800 bg-black/45 px-3.5 py-2.5"
                  >
          ...
          <article className={sectionCardClass}>
            ...
            <h3 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-100">
              Build your next campaign with structured payout execution.
            </h3>
            <p className="mt-3 text-sm leading-7 text-zinc-400">
              Move from brief to funded campaign in minutes, then monitor
              participant quality and settlement from one dashboard.
            </p>
        ...
        <section className={sectionCardClass}>
          <div className="max-w-3xl">
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-400">
              Everything teams usually ask before moving campaign operations
              onto Earnify.
            </p>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr] lg:items-end">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-semibold tracking-tight text-zinc-100 sm:text-4xl">
                Launch your next performance campaign on Earnify.
              </h2>
              <p className="mt-3 text-sm leading-7 text-zinc-300 sm:text-base">
                Build, verify, rank, and settle in one product flow with
                audit-ready payout visibility.
              </p>
            </div>
        ...
        <footer className="border-t border-[var(--color-border)] pt-8">
          <div className="grid gap-6 md:grid-cols-[1.4fr_0.6fr]">
            <div>
              <p className="text-lg font-semibold tracking-[0.08em] text-zinc-100">
                EARNIFY
              </p>
              <p className="mt-2 max-w-xl text-sm leading-6 text-zinc-500">
                Campaign performance infrastructure for founders and creators.
                Built for transparent outcomes and reliable settlement.
              </p>
            </div>
